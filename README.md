...
- ドリンク、フード、会話選択肢の正解を表示させることが出来る。(既定では無効)
- ストッキングを強制的に非表示にすることができる。(既定では無効)
- バーに入る前にキャストの出勤順序を変更できる。(既定では無効)
- チェキ（撮影写真）を高解像度で保存できる。(既定では無効)
- F7キーで衣装・パンツ・ストッキングを自由に切り替えることができる。別キャストの上衣・下衣を移植する衣装チェンジャー機能つき。(既定では有効)
- F9キーで旅行・特別なシーンの所持金UI、ボタンガイド、ラブカウンターを非表示にできる。(所持金非表示は既定で有効)
- 色収差エフェクト（画面端のにじみ）を無効化できる。(既定では無効)
- 胸の形状を平坦化するオプション。キャラクターごとに調整可能。(既定では無効)
